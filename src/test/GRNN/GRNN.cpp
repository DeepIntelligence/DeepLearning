        ...
        if (i == 0) {
          ...
          // of hidden output from lower layer at the same time and
           // hidden output from same layer but at previous time
			...
			hiddenStateLayers.push_back(MultiAddLayer(inputOneDim, inputTwoDim, outputDim, sigmoid,
			...
			hiddenOutputLayers.push_back(ElementMultiAdditionLayer);
			resetGateLayers.push_back(MultiAddLayer(inputOneDim, inputTwoDim, outputDim, sigmoid,
					InitializerBuilder::GetInitializer(w_one_init), InitializerBuilder::GetInitializer(w_two_init), 
					InitializerBuilder::GetInitializer(b_init)));
			udpateGateLayers.push_back(MultiAddLayer(inputOneDim, inputTwoDim, outputDim, sigmoid,
					InitializerBuilder::GetInitializer(w_one_init), InitializerBuilder::GetInitializer(w_two_init), 
					InitializerBuilder::GetInitializer(b_init)));
...
arma::mat RNN::forwardInTime(std::shared_ptr<arma::mat> input) {
    ...
    // first reset Gate
        ...
        if (l == 0) {
			resetGateLayers[l].inputOne = outputLayers_prev_output[l];
			resetGateLayers[l].inputTwo = input;
        } else {
			resetGateLayers[l].inputOne = outputLayers_prev_output[l];
			resetGateLayers[l].inputTwo = hiddenLayers[l - 1].output;
        }
	// second update Gate 
        if (l == 0) {
			updateGateLayers[l].inputOne = outputLayers_prev_output[l];
			updateGateLayers[l].inputTwo = input;
        } else {
			updateGateLayers[l].inputOne = outputLayers_prev_output[l];
			udpateGateLayers[l].inputTwo = hiddenLayers[l - 1].output;
        }
	// third, element operation of previous output and reset gate
	
	
	// fourth, hidden state layer
	
	// fifth, hidden output layer
